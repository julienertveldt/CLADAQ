...
namespace CLADAQ
{
    ...
    public partial class CLADAQ : Form
    {
        ...
        private static int intPlotS = 5* intAcqS;                            // number of points to plot
        ...
        private double[] dblFlowBuff = new double[intBuffS];                        // Medicoat FlowWatch
        ...
        private double dblYMax = 50;
        private double dblYMin = 10;
        ...
        private void Task_2_DoWork()                 //  acquisition thread
        {
            ...
            //simTimer = new System.Threading.Timer(this.OnAcquireTimedEvent, autoEvent2, 2000, 2000);

            //  while (PLC_Con.IsConnected || simulateValues)
            //  {

            //aTimer.Elapsed += OnAcquireTimedEvent;
            //aTimer.Start();
        ...
        public void OnDispTimedEvent(Object source, EventArgs myEventArgs)          // acquisition timer
        {
            ...
            for (int i = 0; i < intBuffS; i++)
            {
                if (strTimeBuff[i] != null)
                {
                    ...
                    chart1.Series[0].Points.AddXY(dp.XValue + i, dblCh1[i]);
                    chart1.Series[1].Points.AddXY(dp.XValue + i, dblCh2[i]);
                } else
                {  
                    TimeSpan t1 = Global.sw.Elapsed;
            ...
            while (chart1.Series[0].Points.Count > intPlotS)
           {
            ...
            if (bAutoSize)
            {
                ...
                chart1.ChartAreas[0].AxisY.Maximum = Math.Round(dp.YValues[0]*1.5) + 1;

                dp = chart1.Series[0].Points.FindMinByValue("Y1", 0);
                chart1.ChartAreas[0].AxisY.Minimum = Math.Sign(dp.YValues[0])*Math.Abs(Math.Round(dp.YValues[0]))-1; //-0.001 quick fix to avoid Ymin = Ymax
            }
            else
            {
        ...
        {
            // System.timer.timer implementation: private void OnSimTimedEvent(Object source, EventArgs myEventArgs) 
            ...
            //string msg = String.Format("Thread ID: {0}\n", thread.ManagedThreadId);
            TimeSpan t1 = Global.sw.Elapsed;
            //dblAxisPos[15] = Math.Sin(t1.TotalSeconds);
            //dblAxisPos[16] = Math.Cos(t1.TotalSeconds);

            //dblPosBuff[intBuffPos] = dblAxisPos[15];
            //intBuffPos += 1;
            //if (intBuffPos > intBuffS-1) { intBuffPos = 0; }          //reset buffer if full

            //double[] dblSimTRange = Array.ConvertAll<int, double>(intSimTRange, x => x/2000);
            //double[] dblDum = Enumerable.Repeat(t1.TotalSeconds, 2000).ToArray();
            //dblDum.Zip(dblSimTRange, (x, y) => x + y);


            //System.timer.timer implementation
            // simTimer.Stop();

            //int intIdxEnd = CLADAQ.intIdxEnd;
            //int[] intSimTRange = CLADAQ.intSimTRange;
            //double[] dblPosBuff = mainForm.dblPosBuff;
            ...
            for (int i = 0; i < intBuffS; i++)
            {
                ...
                nowS = now.Second + (i/intBuffS*0.001)*1.0;

                for (int ii = 0; ii < intAcqChans; ii++)
                {
                    //dblPosBuff[i, ii] = Math.Sin( (nowS) * 6.28 + 0.4 *ii);
            ...
            FormTools.AppendText(mainForm, tbLog, "> " + t1.ToString(@"ss\:fffffff\.") + " :Thread = " + msg + " Value = " + dblPosEnd.ToString() + Environment.NewLine);
        ...
        protected void UpdateBuffer(object sender, EventArgs e)
        {
            ...
            TimeSpan t1 = Global.sw.Elapsed;
            FormTools.AppendText(this, tbLog, "> " + t1.ToString(@"ss\:FFFFFF\.") + " : Acquired buffer from event. Laser fdbck = " + dblLastVal.ToString() + Environment.NewLine);
            ...
            if (listAcqBuffer[b].Count >= (intAcqS))       // write to CSV if CSV buffer is full
            {
                ...
                if (b > intNumBuffs - 1)
                    b = 0;
                FormTools.AppendText(this, tbLog, "> Using buffer :" + b.ToString());
                ...
                if (bPlotOn)
                {
                    ...
                    for (int i = 0; i < intAcqS; i += 1)
                    {
                        ...
                        dummy2 = dummy.Select(x => x.PosX).ToArray()[0];
                        chart1.Series[0].Points.AddXY(intDispCounter, dummy2);
                        dummy2 = dummy.Select(x => x.PosY).ToArray()[0];
