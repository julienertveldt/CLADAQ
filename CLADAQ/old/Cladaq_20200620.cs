...
namespace CLADAQ
{
    ...
    public partial class CLADAQ : Form
    {
        ...
        private static bool bDebugLog = false;
        ...
        public void OnDispTimedEvent(Object source, EventArgs myEventArgs)          // acquisition timer
        {
            ...
            for (int i = 0; i < intBuffS; i++)
            {
                if (strTimeBuff[i] != null)
                {
                    ...
                    chart1.Series[0].Points.AddXY(dp.XValue + 1, dblCh4[i]); //dblCh1
                    chart1.Series[1].Points.AddXY(dp.XValue + 1, dblCh5[i]); //dblCh2
           ...
           /* while (chart2.Series[0].Points.Count > (10*intPlotS))
            {
            ...
            if (bAutoSize)
            {
                try
                {
                    ...
                    chart1.ChartAreas[0].AxisY.Minimum = Math.Sign(dp.YValues[0]) * Math.Abs(Math.Round(dp.YValues[0])) * 1.1 - 1; //-0.001 quick fix to avoid Ymin = Ymax
