...
namespace CLADAQ
{
    ...
    public partial class CLADAQ : Form
    {
        ...
        protected static bool bDebugLog = true;
        ...
        protected static int intBuffS = 5 * 20;                           // position value buffer sent by PLC: same as in GlobVarContant XM22
        protected static int intAcqS = intBuffS * 10;                   // acquisition buffer to be written to file
        protected static int intDispDelay = 1000 / 4;                     // ms delay for screen refresh
        protected static int intAcqDelay = 10;                          // ms delay for acquisition timer
        protected static int intIdxEnd = intBuffS;                      // number of values in pos data buffer
        protected static int intNumBuffs = 2;                           // number of (cyclic) buffers to use.
        protected static int intPlotS = intAcqS / 2;                            // number of points to plot
        protected static int intPlotSkip = 10;                          // plot 1 out of X samples
        ...
        protected int intBuffPos = 0;                                 //current position in the position value buffer
        protected int intAcqBuffPos = 0;                              //current position in the acquired data buffer --> goes in CSV file
        protected int Idcount = 0;
        protected int b = 0;                                          //buffer index
        private int count = 0;                                      //plot index OLD version
        protected int intDispCounter = 0;                             //plot index
        ...
        protected double[,] dblPosBuff = new double[intBuffS, intAcqChans];            // CNC positions + laser channel
        protected string[] strTimeBuff = new string[intBuffS];                        // CNC timestamp
        protected UInt64[] intTimeBuff = new UInt64[intBuffS];
        protected UInt64[] intTimeBuffOld = new UInt64[intBuffS];                     //previous cnc timestamp
        protected string[] strDateBuff = new string[intBuffS];
        ...
        protected DAQBuffer Buffer1;

        // define internal CLADAQ buffers
        protected List<DataRecord>[] listAcqBuffer = new List<DataRecord>[intNumBuffs];                  // data buffer to write
